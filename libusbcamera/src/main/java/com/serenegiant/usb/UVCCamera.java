...
public class UVCCamera {
	...
	public static final int PIXEL_FORMAT_NV21 = 5;		// = YVU420SemiPlanar,NV21，But when saved to jpg, the color is distorted
    ...
    protected long mControlSupports;			// Feature flags supported by camera control
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
    	...
    	try {
			...
			sb.append("Call nativeConnect return value："+result);
...
//    		mNativePtr = 0;	// Don't clear it here because it calls nativeDestroy.
    	}
    	if (mCtrlBlock != null) {
			mCtrlBlock.close();
   			mCtrlBlock = null;
